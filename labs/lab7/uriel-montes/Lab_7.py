...
def populateTable(_conn):
    ...
    try:
        ...
        _conn.executemany(sql, warehouses)
...
def Q1(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q1")
# Q1 displays the entire content of the warehouse table sorted on w_warehousekey 
# by performing a SQL query. (3 points)

    try:
        sql = """SELECT w_warehousekey, w_name, w_capacity, w_suppkey, w_nationkey 
                FROM warehouse
                ORDER BY w_warehousekey"""
        cur = _conn.cursor()
        cur.execute(sql)

        header = '{:<11}{:>6}{:>45}{:>10}{:>10}'.format("wId","wName","wCap","sId","nId")
        print(header)
        header_out = '{:<11}{:>6}{:>45}{:>10}{:>10}\n'.format("wId","wName","wCap","sId","nId")
        with open('output/1.out', 'w') as f:
            f.write(str(header_out))
            f.close()

        rows = cur.fetchall()

        for row in rows:
            data = '{:<12}{:<38}{:>12}{:>10}{:>10}'.format(row[0],row[1],row[2],row[3],row[4])
            print(data)
        with open('output/1.out', 'a') as f:
            for row in rows:
                data_out = '{:<12}{:<38}{:>12}{:>10}{:>10}\n'.format(row[0],row[1],row[2],row[3],row[4])
...
def Q2(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q2")
# Q2 computes the number of warehouses and the total capacity for the warehouses 
# in every nation. The result is sorted in decreasing order of the number of 
# warehouses and of the capacity, then alphabetical order of the nation name (3 points)
    ...
    try:
        ...
        header = '{:<12}{:>12}{:>10}'.format("nation","numW","totCap")
        print(header)
        ...
        for row in rows:
            data = '{:<14}{:>10}{:>10}'.format(row[0],row[1],row[2])
            print(data)
...
def Q3(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q3")
# Q3 computes the suppliers that have a warehouse in a given nation taken
# as input parameter. The nation where warehouses are located is read from the 
# input file input/3.in. Q3 prints the name of the supplier, the nation of the 
# supplier, and the name of the warehouse - sorted in alphabetical order by
# supplier name. (3 points)

    try:
        line1 = 0
        with open('input/3.in', 'r') as filehandle:
            line_num = 1
        ...
        header = '{:<21}{:<20}{:>10}'.format("supplier","nation","warehouse")
        print(header)
        ...
        for row in rows:
            data = '{:<21}{:<20}{:>10}'.format(row[0],row[1],row[2])
            print(data)
...
def Q4(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q4")
# Q4 finds the warehouses from a given region that have capacity larger than a 
# given threshold. The region name and the minimum capcity are parameters stored 
# in the file input/4.in. Q4 prints the warehouse name and its capacity
# in decreasing order of the capacity. (3 points)

    try:
        line1 = 0
        line2 = 0
        with open('input/4.in', 'r') as filehandle:
            line_num = 1
            for line in filehandle:
                if line_num == 1:
                    line1 = line
        ...
        header = '{:<40}{:<1}'.format("warehouse", "capacity")
        print(header)
...
def Q5(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q5")
# Q5 determines the total capacity of the warehouses belonging to suppliers from 
# a given nation in every region. The suppliers' nation is a parameter stored in 
# the file input/5.in. If there are no warehouses in a region, then value 0 is 
# printed for the region. Q5 prints the region and the capacity sorted 
# alphabetically by region. (3 points)

    try:
        line1 = 0
        with open('input/5.in', 'r') as filehandle:
            line_num = 1
            for line in filehandle:
                if line_num == 1:
                    line1 = line
        ...
        sql = """SELECT r_name, SUM(w_capacity)
                FROM region, warehouse, nation, supplier
                WHERE r_regionkey = n_regionkey
                    AND n_nationkey = w_nationkey
                    AND w_suppkey IN (
                                    SELECT s_suppkey
        ...
        header = '{:<20}{:<1}'.format("region", "capacity")
        print(header)
        ...
        for row in rows:
            data = '{:<20}{:<1}'.format(row[0],row[1])
            data = data.rstrip('00')
            print(data)
        with open('output/5.out', 'a') as f:
            for row in rows:
                data_out = '{:<20}{:<1}'.format(row[0],row[1])
                data_out = data_out.rstrip('00')
                f.write(str(data_out))
                f.write(str('\n'))
        f.close()
